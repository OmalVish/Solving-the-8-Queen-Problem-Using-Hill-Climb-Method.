...
class queens:
    ...
    def hill_solution(self):
        while 1:
            ...
            if self.verbocity == True:
                print ("Violations Found"), self.calc_cost(self.mboard)
                print (self.mboard)
        if self.cost != 0:
            if self.verbocity == True:
                print ("Local Maximums Found (No Perfect Solution)")
        else:
            if self.verbocity == True:
                print ("Global Maximum Found (Perfect Solution Found)")
            self.totalsucc += 1
        return self.cost

    def printstats(self):
        print ("Total Number of Runs: "), self.totalruns
        print ("Total Number of Success: "), self.totalsucc
